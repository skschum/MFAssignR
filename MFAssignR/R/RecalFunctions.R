...
#'
#' @examples
#' Recal(df = Data, peaks = Mono, isopeaks = Iso, mode = "neg", SN = 500, series1 = "O4_H_2",
#' series2 = "O4_H_8", series3 = "O6_H_8")
#'
#' Recal(df = Data, peaks = Mono, isopeaks = Iso, mode = "pos", SN = 300, series1 = "O4_Na_2",
...
if(cols ==3){
  ...
  #Monoisotopic Peaks
  #Prepares the recalibrant masses for use in recalibration steps.
  RecalList <- RecalList2[c("Abundance", "Exp_mass", "RT", "C", "H", "O", "N", "S", "P", "E",
                            "S34", "N15", "D", "Cl", "Fl", "Cl37", "M", "NH4", "POE", "NOE", "Z", "C13_mass")]
  RecalList$NM <- round(RecalList$Exp_mass)
...
if(cols == 3){NewRecalx <- NewRecal[c(1:6)]}  #LCMS
...
if(cols == 3){FinalRecal2x <- FinalRecal2[c(1:7)]} #LCMS
FinalRecal2 <- FinalRecal2x  #LCMS

if(cols ==2) {names(FinalRecal2)[3] <- "formula"}  #LCMS
if(cols ==3) {names(FinalRecal2)[4] <- "formula"; names(FinalRecal2)[3] <- "RT"}  #LCMS
#########################

...
IsoFiltR <- function(peaks, SN = 0, Carbrat = 60, Sulfrat = 30, Sulferr = 5, Carberr = 5) {
  ...
  if(cols == 3){names(peaks)[3] <- "RT"}
  if(cols == 2){peaks$RT <- 1:nrow(peaks)}
...
#i <- 1
  ...
  Sulfs <- merge(MonoOutC_final, MonoOutS_final, by.x = c("Exp_mass", "Abundance", "RT"),
  ...
  Carbs1 <- merge(IsoOutC1_final, Sulfbad, by.x = c("Exp_mass", "Abundance", "RT"),
                 by.y = c("Exp_mass", "Abundance", "RT"), all = TRUE)
  ...
  Sulfbad <- Sulfbad[-4]
  Carbs2 <- merge(IsoOutC2_final, Sulfbad, by.x = c("Exp_mass", "Abundance", "RT"),
                  by.y = c("Exp_mass", "Abundance", "RT"), all = TRUE)
  ...
  All_check <- merge(End, All_check, by.x = c("exp_mass", "abundance", "RT"),
  ...
  Iso_final <- Iso_final[!is.na(All_check$tag),]

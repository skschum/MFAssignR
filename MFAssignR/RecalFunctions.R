...
#'
#' @param opt numeric - selects the recalibration option to be used, options are 1, 2, 3, or 4
#'
...
Recal <- function(df, peaks, isopeaks = "none",mode, SN = 0, mzRange = 50, series1=NA, series2=NA, series3=NA, series4=NA, series5=NA,
  ...
  RecalList <- RecalList2[c("Abundance", "Exp_mass", "C", "H", "O", "N", "S", "P", "E",
                            "S34", "N15", "D", "Cl", "Fl", "Cl37", "M", "NH4", "POE", "NOE", "Z", "C13_mass", "series")]
  ...
  Step2 <- Step2[-c(10,32)] #Removes Abundance.y and O_num
  ...
  NewRecal <- NewRecal[c("Abundance", "Exp_mass", "form", "theor_mass", "C13_mass", "series")]
  #Good to here
  #####################
  ...
  ##Opening this up again
   # NewRecal_mono <- FinalRecal2[c(1:4)]
   # NewRecal_iso <- FinalRecal2[c(1,5,3,4)]
   # NewRecal_iso <- NewRecal_iso[NewRecal_iso$C13_mass > 0,]
   # names(NewRecal_iso)[2] <- "Exp_mass"
   # NewRecal_iso$theor_mass <- NewRecal_iso$theor_mass + 1.0033548380
   # NewRecal <- rbind(NewRecal_mono, NewRecal_iso)
   ##To see if it still works
  NewRecal <- FinalRecal2
###############################################################
  ...
  names(FullRecal)[2] <- "E_mass"
  names(FullRecal)[4] <- "Th_mass"
  ...
  FullRecal$mzweight <- FullRecal$weight*FullRecal$E_mass  #Serves very little purpose
  ...
  FullRecal$masssum <- sum(FullRecal$mzweight)/2^(nrow(FullRecal)-1)  #Unimportant
  FullEjsum <- mean(FullRecal$Ejsum)  #Full spectrum correction term
  ...
  peaks_full$mass <- peaks_full$mass/(1+FullEjsum)  ##Performs the initial recalibration of the full spectrum

  df_abund <- peaks_full[!is.na(peaks_full$form),]
  df_abund <- df_abund[c("mass", "Abundance", "form", "Tag2")]  #Pulls out the recalibrated peaks that were assigned

  df1 <- merge(df_abund, Forms2, by.x = "form", by.y = "form") #Creates pseudo input dataframe for the next round of calibration
  df1 <- unique(df1)
  df1$Adduct <- "H"
  ...
  RecalList3 <- merge(df1, RecalList_keep, by.x = "series", by.y = "series")
  recal_forms <- FullRecal[c("form", "Abundance", "masssum")]  #Isolating the recalibration formulas
  RecalList3 <- merge(df1, recal_forms, by.x = "form", by.y = "form")
  RecalList3 <- RecalList3[RecalList3$Abundance.x == RecalList3$Abundance.y,]
#save1 <- RecalList3
  ##Breaking the spectrum into pieces for the piecewise recalibration
  pieces <- list()
  RecalList3 <- RecalList3[order(RecalList3$Exp_mass),]
  #names(RecalList3)[2] <- "Exp_mass"
...
#dfx <- pieces[[35]]
  ...
  SubRecal <- function(dfx){
    ...
    #ensures that chosen calibrants are at least 1% of the base recalibrant in the section
    low_recal <- low_recal[low_recal$Abundance >= 0.01 * recalpeak$Abundance,]
    high_recal <- high_recal[high_recal$Abundance >= 0.01 * recalpeak$Abundance,]
